...
def creandoArchivo():
    dicDatos = open("diccionarioDatos.txt", "w")
...
def distancia_levenshtein(palabra1, palabra2):
    ...
    if len(palabra1) < len(palabra2):
        return distancia_levenshtein(palabra2, palabra1)
...
def buscarTrad(palabra, listEspa, listIng):
    ...
    for indice in range(len(listEspa)): #En español e ingles tiene el mismo tamaño la lista
        porceEsp = porcentaje_similitud(palabra, listEspa[indice])
        porceIng = porcentaje_similitud(palabra, listIng[indice])
    ...
    if (flag == 0):    
        ...
        erroresLista(similitudEsp, similitudIng, list1, list2, palabra)
...
def erroresLista(similitudEsp, similitudIng, listEspa, listIng, palabra):
    ...
    if(opc == 0):
        ...
        if(idioma == 1):
            agregaP(palabra, traduccion)
        if(idioma == 2):
            agregaP(traduccion, palabra)    
    else:
        idioma = input("¿En qué idioma esta la palabra? 1=Esp 2=Ing : ")    
        idioma = int(idioma)
        if(idioma == 1):
...
def menu(diccionario):
    ...
    listEspa, listIng = listasTabla(diccionario)
    #Comparar palabra con las listas (75% de concordancia)
    buscarTrad(palabra, listEspa, listIng)
